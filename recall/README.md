...
사이트를 열면 `data/sentences.json`의 공개 데이터베이스를 자동으로 불러옵니다. 이 데이터는 로그인이나 토큰 없이 모든 기기에서 볼 수 있습니다.

브라우저에서 새로 등록한 문장은 우선 그 기기의 `localStorage`에 저장됩니다. 컴퓨터에서만 문장을 입력하고 모든 기기에서 보이게 하려면, 컴퓨터 브라우저의 `GitHub 게시` 카드에 게시용 토큰을 한 번 저장하세요. 그 뒤 컴퓨터에서 문장 등록, 수정, 삭제, 복습 완료를 하면 `recall/data/sentences.json` 공개 DB가 자동 업데이트됩니다.

### 컴퓨터 게시 설정

1. GitHub에서 `Settings > Developer settings > Personal access tokens`로 이동합니다.
2. Fine-grained token을 만들고, `ftstudent/LOGOSTUDY` 저장소의 `Contents` 권한을 `Read and write`로 설정합니다.
3. 앱의 `GitHub 게시` 카드에 토큰을 입력합니다.
4. 저장소는 `ftstudent/LOGOSTUDY`, 브랜치는 `main` 그대로 둡니다.
5. `설정 저장`을 누릅니다.

이 설정은 컴퓨터 브라우저에만 필요합니다. 폰과 아이패드는 아무 설정 없이 공개 DB를 읽습니다. `지금 게시`나 `강제 게시`는 현재 컴퓨터 데이터를 공개 DB 파일에 즉시 반영합니다.
